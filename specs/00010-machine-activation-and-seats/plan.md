...
| AD-008 | Rate limiting | hand-rolled / `@fastify/rate-limit` | `@fastify/rate-limit` (in-memory) scoped to BOTH runtime activation routes (activate + deactivate), keyed per API key (the limiter runs at `onRequest`, before the body is parsed, so the license id is not yet known — per-key is the correct actor granularity anyway); over-limit → `429 rate_limited` + `Retry-After`, audited (FR-020); default 60 req/min per API key | Idiomatic, single-install; distributed store (Redis) deferred to scale-out |
