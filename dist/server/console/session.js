...
export async function createSession(pool, tenantId, userId, ttlSeconds) {
    const token = generateToken();
    const id = randomUUID();
    const expiresAt = new Date(Date.now() + ttlSeconds * 1000);
    await withTenant(pool, tenantId, (q) => q(`INSERT INTO admin_session (id, tenant_id, user_id, token_hash, expires_at, last_seen_at)
       VALUES ($1, current_setting('app.current_tenant')::uuid, $2, $3, $4, now())`, [id, userId, tokenHash(token), expiresAt]));
...
export async function revokeSession(pool, tenantId, sessionId) {
    await withTenant(pool, tenantId, (q) => q("UPDATE admin_session SET revoked_at = now() WHERE id = $1 AND revoked_at IS NULL", [sessionId]));
}
