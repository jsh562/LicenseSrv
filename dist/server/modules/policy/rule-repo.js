...
const RULE_COLUMNS = "id, rule_key, version, entitlement_id, plan_id, condition, effect, priority, status, author, created_at, updated_at";
/**
...
export class PolicyRuleRepo {
     ...
     */
    async insertVersion(q, input) {
        const id = input.id ?? randomUUID();
        const r = await q(`INSERT INTO policy_rule
     ...
     */
    async nextVersion(q, ruleKey) {
        const r = await q("SELECT COALESCE(MAX(version), 0)::int AS v FROM policy_rule WHERE rule_key = $1", [ruleKey]);
        return r.rows[0].v + 1;
    }
    /**
     ...
     */
    async updateStatus(q, p) {
        const r = await q(`UPDATE policy_rule SET status = $3, updated_at = now()
        WHERE rule_key = $1 AND version = $2
        RETURNING ${RULE_COLUMNS}`, [p.ruleKey, p.version, p.status]);
        if ((r.rowCount ?? 0) === 0)
            return null;
        return mapRule(r.rows[0]);
    }
    /** Read one specific rule version (or null). RLS scopes it to the tenant (cross-tenant → not found, FR-015). */
    async getVersion(q, ruleKey, version) {
        const r = await q(`SELECT ${RULE_COLUMNS} FROM policy_rule WHERE rule_key = $1 AND version = $2`, [ruleKey, version]);
        if ((r.rowCount ?? 0) === 0)
            return null;
        return mapRule(r.rows[0]);
    }
    /** The single live (active|preview) version of a logical rule, or null (INV-3 guarantees at most one). */
    async getLiveVersion(q, ruleKey) {
        const r = await q(`SELECT ${RULE_COLUMNS} FROM policy_rule
        WHERE rule_key = $1 AND status IN ('active','preview')
        ORDER BY version DESC LIMIT 1`, [ruleKey]);
        if ((r.rowCount ?? 0) === 0)
            return null;
        return mapRule(r.rows[0]);
    }
     ...
     */
    async getVersions(q, ruleKey) {
        const r = await q(`SELECT ${RULE_COLUMNS} FROM policy_rule WHERE rule_key = $1 ORDER BY version DESC`, [ruleKey]);
    ...
    async listLatestRules(q, f) {
        ...
        params.push(f.limit);
        const r = await q(`SELECT ${RULE_COLUMNS} FROM (
     ...
     */
    async countLiveRulesForTenant(q) {
        const r = await q("SELECT count(*)::int AS n FROM policy_rule WHERE status IN ('active','preview')");
        return r.rows[0].n;
    }
    /** Count the LIVE (active|preview) rule versions targeting one entitlement (the FR-019 per-entitlement cap). */
    async countLiveRulesForEntitlement(q, entitlementId) {
        const r = await q("SELECT count(*)::int AS n FROM policy_rule WHERE entitlement_id = $1 AND status IN ('active','preview')", [entitlementId]);
        return r.rows[0].n;
    }
    /**
     ...
     */
    async appendEvaluation(q, input) {
        const id = input.id ?? randomUUID();
        await q(`INSERT INTO policy_evaluation
