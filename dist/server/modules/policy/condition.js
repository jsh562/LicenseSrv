 ...
 */
function resolveVar(pathArg, data, state) {
    let path;
    let dflt;
...
function finiteOrThrow(n) {
    if (!Number.isFinite(n))
        throw new ConditionError("arithmetic_error", "non-finite arithmetic result");
    return n;
}
/** Apply a single allow-listed operator. `and`/`or`/`if` short-circuit (so a `has()`-guard protects a `var`). */
function applyOp(op, arg, data, state) {
    switch (op) {
        case "var":
        ...
        case "and": {
            for (const node of asArray(arg)) {
                if (!truthy(evalNode(node, data, state)))
                    return false;
            }
            return true;
        }
        ...
        case "if": {
            ...
            for (; i + 1 < list.length; i += 2) {
                if (truthy(evalNode(list[i], data, state)))
                    return evalNode(list[i + 1], data, state);
            }
...
function applyValueOp(op, args) {
    ...
    switch (op) {
        ...
        case "in": {
            if (typeof b === "string")
                return typeof a === "string" && b.includes(a);
            if (Array.isArray(b))
                return b.some((x) => x === a);
            return false;
        }
        case "+":
            return finiteOrThrow(args.reduce((acc, x) => acc + toNum(x), 0));
        case "*":
            return finiteOrThrow(args.reduce((acc, x) => acc * toNum(x), 1));
        case "-":
...
function evalNode(node, data, state) {
    state.ops += 1;
    ...
    if (Array.isArray(node))
        return node.map((n) => evalNode(n, data, state));
    ...
    const result = applyOp(op, node[op], data, state);
    state.depth -= 1;
    return result;
}
/**
 ...
 */
export function evaluateCondition(condition, context, opts = {}) {
    ...
    const data = opts.now != null && !Object.prototype.hasOwnProperty.call(context, "now")
