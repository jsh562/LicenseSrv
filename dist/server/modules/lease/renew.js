...
import { buildGrant, maybeSignHandle, resolveLicenseForLease } from "./acquire.js";
 ...
 */
export async function renewLease(deps, tenantId, leaseId) {
    const { pool, repo, signer } = deps;
    return withTenant(pool, tenantId, async (q) => {
        const existing = await repo.getById(q, leaseId);
        if (!existing)
            throw new LeaseError("not_found", 404, "unknown lease", { leaseId });
...
function diagnose(status, expiresAt) {
    if (status !== "live")
        return "reclaimed"; // released or reclaimed — a terminal seat
