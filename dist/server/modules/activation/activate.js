...
import { chooseMatch, deriveMachineId } from "./fingerprint.js";
 ...
 */
export async function activate(pool, signer, config, tenantId, input) {
    ...
    if (!signer)
        throw new ActivationError("signer_unavailable", 503, "no signer is configured");
    ...
    return withTenant(pool, tenantId, async (q) => {
        ...
        if (!licenseId) {
            ...
            if (!lr.rowCount)
                throw new ActivationError("license_not_found", 404, "unknown license");
        ...
        const priorNonce = await q("SELECT id, license_id, machine_id, activated_at, machine_bound_token FROM activation WHERE nonce = $1", [input.nonce]);
        if (priorNonce.rowCount) {
            const row = priorNonce.rows[0];
        ...
        const activeRes = await q("SELECT id, machine_id, signal_hashes, updated_at FROM activation WHERE license_id = $1 AND status = 'active'", [licenseId]);
        const candidates = activeRes.rows.map((r) => ({
        ...
        try {
            ...
            if (e instanceof SignerError)
                throw new ActivationError("signer_unavailable", 503, `signer unavailable (${e.failure})`);
            throw e;
        }
        try {
            if (match) {
                // Re-activation (drift or same machine): refresh in place — same seat, new credential (FR-005).
                const upd = await q(`UPDATE activation
             SET machine_id = $2, signal_hashes = $3, fp_min = $4, nonce = $5, machine_bound_token = $6,
                 label = COALESCE($7, label), updated_at = now()
           WHERE id = $1 RETURNING id, activated_at`, [match.id, machineId, input.signals, config.fpMin, input.nonce, token, input.label ?? null]);
            ...
            const id = randomUUID();
            const ins = await q(`INSERT INTO activation (id, tenant_id, license_id, machine_id, signal_hashes, fp_min, nonce, machine_bound_token, label)
         VALUES ($1, current_setting('app.current_tenant')::uuid, $2, $3, $4, $5, $6, $7, $8)
         RETURNING id, activated_at`, [id, licenseId, machineId, input.signals, config.fpMin, input.nonce, token, input.label ?? null]);
