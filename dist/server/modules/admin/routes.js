...
const scopeSchema = z.enum(["activate", "validate", "lease", "usage.ingest", "admin"]);
 ...
 */
export function registerAdminRoutes(app, pool, config) {
    const ttl = config.sessionTtlSeconds;
    const sessionCookie = (maxAgeSeconds) => ({ httpOnly: true, secure: true, sameSite: "strict", path: "/admin", maxAge: maxAgeSeconds });
    const csrfCookie = (maxAgeSeconds) => ({ httpOnly: false, secure: true, sameSite: "strict", path: "/admin", maxAge: maxAgeSeconds });
    // --- Auth (US1) ---------------------------------------------------------------------------------
    ...
    app.post("/admin/auth/login", async (req, reply) => {
        ...
        if (!parsed.success)
            return err(reply, 400, "validation_error", "invalid login payload");
    ...
    app.post("/admin/users", { preHandler: requireRole(pool, "admin") }, async (req, reply) => {
        ...
        if (!parsed.success)
            return err(reply, 400, "validation_error", "invalid user payload");
        const a = req.admin;
        const outcome = await createUser(pool, config.secret, a.tenantId, a.userId, parsed.data);
        if (!outcome.ok)
            return err(reply, 409, "duplicate_user", "a user with that email already exists");
    ...
    app.patch("/admin/users/:userId", { preHandler: requireRole(pool, "admin") }, async (req, reply) => {
        ...
        if (!p.success)
            return err(reply, 400, "validation_error", "invalid userId");
        const body = updateUserSchema.safeParse(req.body);
        if (!body.success)
            return err(reply, 400, "validation_error", "invalid update payload");
        const a = req.admin;
        const outcome = await updateUser(pool, a.tenantId, a.userId, p.data.userId, body.data);
        if (!outcome.ok) {
            if (outcome.reason === "not-found")
                return err(reply, 404, "user_not_found", "unknown user");
    ...
    app.post("/admin/api-keys", { preHandler: requireRole(pool, "admin") }, async (req, reply) => {
        ...
        if (!parsed.success)
            return err(reply, 400, "validation_error", "invalid api-key payload");
        const a = req.admin;
        const created = await createApiKey(pool, config.secret, a.tenantId, a.userId, parsed.data.scopes);
    ...
    app.post("/admin/api-keys/:keyId/rotate", { preHandler: requireRole(pool, "admin") }, async (req, reply) => {
        ...
        if (!p.success)
            return err(reply, 400, "validation_error", "invalid keyId");
        const a = req.admin;
        const rotated = await rotateApiKey(pool, config.secret, a.tenantId, a.userId, p.data.keyId);
        if (!rotated)
            return err(reply, 404, "key_not_found", "unknown api key");
    ...
    app.post("/admin/api-keys/:keyId/revoke", { preHandler: requireRole(pool, "admin") }, async (req, reply) => {
        ...
        if (!revoked)
            return err(reply, 404, "key_not_found", "unknown or already-revoked api key");
    ...
    app.get("/admin/audit", { preHandler: requireRole(pool, "viewer") }, async (req, reply) => {
        ...
        if (!q.success)
            return err(reply, 400, "validation_error", "invalid audit filters");
