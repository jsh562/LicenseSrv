...
export function registerIssuanceRoutes(app, pool, deps) {
    ...
    app.post("/admin/customers", admin, async (req, reply) => {
        ...
        if (!b.success)
            return validation(reply, "invalid customer payload");
    ...
    app.get("/admin/customers/:customerId", viewer, async (req, reply) => {
        ...
        if (!p.success)
            return validation(reply, "invalid customerId");
    ...
    app.get("/admin/licenses", viewer, async (req, reply) => {
        ...
        if (!q.success)
            return validation(reply, "invalid license filters");
    ...
    // --- Lifecycle ----------------------------------------------------------------------------------
    const lifecycle = (action, fn) => app.post(`/admin/licenses/:licenseId/${action}`, admin, async (req, reply) => {
        const p = licenseParams.safeParse(req.params);
        if (!p.success)
            return validation(reply, "invalid licenseId");
    ...
    app.post("/admin/licenses/:licenseId/transfer", admin, async (req, reply) => {
        ...
        if (!b.success)
            return validation(reply, "a target customerId is required");
        return guard(reply, async () => reply.code(200).send(await transferLicense(pool, req.admin.tenantId, req.admin.userId, p.data.licenseId, deps.config.transferLimit, b.data.customerId)));
    });
}
