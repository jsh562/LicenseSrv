...
import { LICENSE_SELECT, mapLicenseRow } from "./licenses.js";
/** Load a license's status + transfer count under a row lock, or 404. */
async function lockLicense(q, id) {
    const r = await q("SELECT status, transfer_count FROM license WHERE id = $1 FOR UPDATE", [id]);
    if (!r.rowCount)
        throw new IssuanceError("not_found", 404, "unknown license");
    return r.rows[0];
}
async function updateReturning(q, id, set, params) {
 ...
 */
export async function transferLicense(pool, tenantId, actor, id, transferLimit, newCustomerId) {
    return withTenant(pool, tenantId, async (q) => {
        const cur = await lockLicense(q, id);
        if (cur.status === "revoked")
            throw new IssuanceError("invalid_transition", 409, "a revoked license cannot be transferred");
        ...
        if (!target.rowCount)
            throw new IssuanceError("not_found", 404, "unknown target customer");
