 ...
 */
export async function onboardReseller(deps, params, actorUserId) {
    ...
    await withTenant(deps.pool, tenantId, (q) => writeAudit(q, {
        actor: actorUserId,
 ...
 */
export async function provisionSubTenant(deps, params) {
    ...
    await withTenant(deps.pool, id, (q) => writeResellerAudit(q, {
 ...
 */
export async function suspendReseller(deps, resellerTenantId, actorUserId) {
    ...
    await withTenant(deps.pool, resellerTenantId, (q) => writeAudit(q, {
        actor: actorUserId,
 ...
 */
export async function reinstateReseller(deps, resellerTenantId, actorUserId) {
    const reseller = await deps.repo.getReseller(resellerTenantId);
    if (!reseller) {
        throw new ResellerError("not_found", 404, "reseller not found", { resellerId: resellerTenantId });
    }
 ...
 */
export async function offboardReseller(deps, resellerTenantId, actorUserId) {
    const reseller = await deps.repo.getReseller(resellerTenantId);
    if (!reseller) {
        throw new ResellerError("not_found", 404, "reseller not found", { resellerId: resellerTenantId });
    }
 ...
 */
export async function moveSubTenant(deps, params) {
    ...
    const meta = await privileged(deps.pool, async (q) => {
        const r = await q("SELECT id, slug, name, parent_reseller_id, deleted_at, created_at FROM tenant WHERE id = $1 AND deleted_at IS NULL", [subTenantId]);
        return r.rowCount
            ? r.rows[0]
            : null;
    });
    ...
    let destinationResellerId = null;
    ...
    const lockedAtDestination = resolved.filter((f) => f.locked).map((f) => f.field);
