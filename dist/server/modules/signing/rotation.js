 ...
 */
export async function rotateKey(pool, tenantId, productId, custody, actor, overlapSeconds = 2_592_000) {
    if (!custody.unlocked)
        throw new Error("custody locked: cannot rotate");
    ...
    return withTenant(pool, tenantId, async (q) => {
        ...
        await q(`UPDATE signing_key
          SET status = 'rotating', valid_until = now() + ($2::int * interval '1 second')
        WHERE product_id = $1 AND status = 'active'`, [productId, overlapSeconds]);
       ...
       VALUES ($1, current_setting('app.current_tenant')::uuid, $2, $3, 'ed25519', $4, 'active',
               now(), $5, $6, NULL)
       RETURNING key_id, product_id, algorithm, public_key, status, valid_from, valid_until, created_at`, [id, productId, gen.keyId, gen.publicKey, wrapped, KEYSTORE_SCHEME]);
        await writeAudit(q, {
            actor,
...
export async function revokeKey(pool, tenantId, productId, keyId, actor) {
    return withTenant(pool, tenantId, async (q) => {
        const r = await q(`UPDATE signing_key SET status = 'revoked', valid_until = now()
         WHERE product_id = $1 AND key_id = $2 AND status <> 'revoked'`, [productId, keyId]);
...
export async function retireKey(pool, tenantId, productId, keyId, actor) {
    return withTenant(pool, tenantId, async (q) => {
        const r = await q("UPDATE signing_key SET status = 'retired' WHERE product_id = $1 AND key_id = $2 AND status = 'rotating'", [productId, keyId]);
        const changed = (r.rowCount ?? 0) > 0;
        if (changed) {
