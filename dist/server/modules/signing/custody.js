...
export function shamirCombine(shares) {
    if (shares.length < 2)
        throw new Error("shamir: need at least the threshold of shares");
    const len = shares[0].length - 1;
    const xs = shares.map((s) => s[0]);
    if (new Set(xs).size !== xs.length)
        throw new Error("shamir: duplicate share indices");
...
export function wrapKey(masterKey, plaintext) {
...
export function unwrapKey(masterKey, blob) {
    if (masterKey.length !== 32)
        throw new Error("master key must be 32 bytes");
...
export class Custody {
    ...
    unlock(shares) {
        ...
        if (master.length !== 32)
            throw new Error("reconstructed master key must be 32 bytes");
    ...
    wrap(privateKey) {
        if (!this.#masterKey)
            throw new Error("custody locked");
