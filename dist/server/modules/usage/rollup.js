 ...
 */
export function sumNet(quantities) {
 ...
 */
export function uniqueCount(hashes) {
    const seen = new Set();
 ...
 */
export function floorDisplay(net) {
 ...
 */
export function isOverQuota(net, allowance) {
...
function canonicalDimensions(dimensions) {
 ...
 */
export function hashDimensions(dimensions) {
...
export function bucketStartFor(eventTime, bucketSeconds) {
 ...
 */
export function classifyAcceptance(eventTime, now, cfg) {
    const t = eventTime.getTime();
    if (t < now.getTime() - cfg.retentionSecs * 1000)
        return "stale";
    if (t > now.getTime() + cfg.futureSkewSecs * 1000)
        return "future";
    return "ok";
}
/**
 ...
 */
export async function rollupBucket(q, repo, input) {
    ...
    const prev = await q("SELECT over_quota FROM usage_rollup WHERE license_id = $1 AND entitlement_id = $2 AND bucket = $3", [licenseId, entitlementId, bucketStart]);
    const prevOverQuota = prev.rowCount ? prev.rows[0].over_quota : false;
