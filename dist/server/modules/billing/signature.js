...
function computeHmac(secret, timestamp, rawBody) {
...
function constantTimeMatch(expected, candidateHex) {
    let candidate;
    try {
...
function anyMatch(secret, header, rawBody) {
    ...
    for (const sig of header.signatures)
        matched = constantTimeMatch(expected, sig) || matched;
    return matched;
}
/**
 ...
 */
export function verifySignature(rawBody, header, secretCurrent, secretPrev, nowUnix, toleranceSecs) {
    if (!header)
    ...
    if (-skew > toleranceSecs)
        return { ok: false, reason: "future_timestamp", timestamp: parsed.timestamp };
