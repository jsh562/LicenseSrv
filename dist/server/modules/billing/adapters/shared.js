...
import { buildPayloadSummary } from "../events.js";
...
export function extractSubscriptionId(object) {
    ...
    if (typeof sub === "object" && sub !== null)
        return asString(sub.id);
    return (asString(object.subscriptionId) ??
...
export function extractPlanKey(object) {
 ...
 */
export function extractCustomerId(object) {
    const customer = object.customer;
    if (typeof customer === "string")
        return asString(customer);
...
export function extractPeriodEnd(object) {
    const raw = asNumber(object.current_period_end) ??
 ...
 */
export function buildCanonicalEvent(provider, envelope, canonicalType) {
    ...
    const paymentStatus = asString(envelope.object.paymentStatus) ?? asString(envelope.object.payment_status) ?? null;
