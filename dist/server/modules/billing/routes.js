 ...
 */
async function auditWebhookRateLimited(deps, req) {
    const params = req.params;
    const connectionId = params?.connectionId;
    if (!connectionId || !UUID_RE.test(connectionId))
        return; // unknown/invalid id → no tenant to scope
    const owner = await privileged(deps.pool, (q) => q("SELECT tenant_id FROM billing_connection WHERE id = $1", [connectionId]));
    if (!owner.rowCount)
        return;
    const tenantId = owner.rows[0].tenant_id;
    await withTenant(deps.pool, tenantId, (q) => recordSecurityEvent(q, {
 ...
 */
async function listSubscriptionSummaries(q, filters) {
    const clauses = [];
    const params = [];
    ...
    const r = await q(`SELECT s.id, s.provider, s.external_subscription_id, s.license_id, l.status AS license_status,
 ...
 */
async function validatePlanMap(deps, tenantId, planMap) {
...
export function registerBillingRoutes(app, deps) {
    ...
    void app.register(async (scope) => {
        ...
        scope.post("/v1/billing/webhooks/:connectionId", async (req, reply) => {
    ...
    void app.register(async (adminScope) => {
        ...
        adminScope.post("/admin/billing/connections", admin, async (req, reply) => {
            ...
            if (!b.success)
                return validation(reply, "invalid connection payload");
        ...
        adminScope.patch("/admin/billing/connections/:id", admin, async (req, reply) => {
            ...
            if (!p.success)
                return validation(reply, "invalid connection id");
            const b = updateConnectionSchema.safeParse(req.body);
            if (!b.success)
                return validation(reply, "invalid connection update");
            return guard(reply, async () => {
                if (b.data.planMap !== undefined)
                    await validatePlanMap(deps, req.admin.tenantId, b.data.planMap);
        ...
        adminScope.post("/admin/billing/connections/:id/rotate-secret", admin, async (req, reply) => {
            ...
            if (!b.success)
                return validation(reply, "a new signingSecret is required");
            return guard(reply, async () => {
        ...
        adminScope.get("/admin/billing/subscriptions", viewer, async (req, reply) => {
            ...
            if (!qy.success)
                return validation(reply, "invalid subscription filter");
        ...
        adminScope.get("/admin/billing/events", viewer, async (req, reply) => {
            ...
            if (!qy.success)
                return validation(reply, "invalid event filter");
        ...
        adminScope.post("/admin/billing/reconcile", admin, async (req, reply) => {
            ...
            if (!b.success)
                return validation(reply, "invalid reconcile scope");
            ...
            return guard(reply, async () => {
                ...
                if (b.data.connectionId) {
                    ...
                    if (!conn)
                        return err(reply, 404, "not_found", "unknown connection", { connectionId: b.data.connectionId });
                }
                if (b.data.subscriptionId) {
                    const exists = await subscriptionExists(deps, tenantId, b.data.subscriptionId);
                    if (!exists)
                        return err(reply, 404, "not_found", "unknown subscription", { subscriptionId: b.data.subscriptionId });
                }
                ...
                void reconcile(deps, deps.providerFetch, { tenantId, connectionId: b.data.connectionId, subscriptionId: b.data.subscriptionId }, { jobId, logger: adminScope.log }).catch((e) => adminScope.log.warn({ error: e instanceof Error ? e.message : String(e) }, "reconcile job failed (fail-open)"));
...
function subscriptionExists(deps, tenantId, subscriptionId) {
