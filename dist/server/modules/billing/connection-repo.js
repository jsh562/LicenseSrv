...
import { isRotationWindowOpen } from "./config.js";
...
const PUBLIC_SELECT = "id, provider, status, secret_custody_scheme, secret_rotated_at, plan_map, default_grace_seconds, grace_overrides, created_at, updated_at";
...
export class ConnectionRepo {
    ...
    create(tenantId, actor, input) {
        ...
        return withTenant(this.pool, tenantId, async (q) => {
            let r;
            try {
                r = await q(`INSERT INTO billing_connection
    ...
    list(tenantId, cap = 1000) {
        return withTenant(this.pool, tenantId, async (q) => {
            const r = await q(`SELECT ${PUBLIC_SELECT} FROM billing_connection_public ORDER BY created_at DESC, id DESC LIMIT $1`, [cap]);
     ...
     */
    update(tenantId, actor, id, input) {
        return withTenant(this.pool, tenantId, async (q) => {
            const sets = [];
            const params = [id];
            ...
            if (input.planMap !== undefined)
                push("plan_map = $$::jsonb", JSON.stringify(input.planMap));
            if (input.defaultGraceSeconds !== undefined)
                push("default_grace_seconds = $$", input.defaultGraceSeconds);
            if (input.graceOverrides !== undefined)
                push("grace_overrides = $$::jsonb", JSON.stringify(input.graceOverrides));
            ...
            if (sets.length === 0) {
                ...
                if (!cur.rowCount)
                    throw new BillingError("connection_not_found", 404, "unknown connection", { connectionId: id });
                return toPublic(cur.rows[0]);
            }
            const r = await q(`UPDATE billing_connection SET ${sets.join(", ")}, updated_at = now() WHERE id = $1 RETURNING ${PUBLIC_SELECT}`, params);
            if (!r.rowCount)
                throw new BillingError("connection_not_found", 404, "unknown connection", { connectionId: id });
     ...
     */
    rotateSecret(tenantId, actor, id, newSecret) {
        return withTenant(this.pool, tenantId, async (q) => {
            const scheme = (await this.currentScheme(q, id)) ?? KEYSTORE_SCHEME;
            const wrapped = this.wrapSecret(scheme, newSecret);
            const r = await q(`UPDATE billing_connection
            SET signing_secret_prev = signing_secret_ref,
                signing_secret_ref  = $2,
                secret_rotated_at   = now(),
                updated_at          = now()
          WHERE id = $1
          RETURNING ${PUBLIC_SELECT}`, [id, wrapped]);
            if (!r.rowCount)
                throw new BillingError("connection_not_found", 404, "unknown connection", { connectionId: id });
     ...
     */
    async resolveSecrets(q, id) {
        const r = await q(`SELECT id, provider, status, signing_secret_ref, signing_secret_prev, secret_custody_scheme,
              secret_rotated_at, plan_map, default_grace_seconds, grace_overrides
         FROM billing_connection WHERE id = $1`, [id]);
        if (!r.rowCount)
            return null;
        const row = r.rows[0];
