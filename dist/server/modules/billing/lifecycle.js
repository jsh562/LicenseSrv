...
import { applySubscriptionState, linkSubscription, resolveSubscriptionByExternalId } from "./subscription-repo.js";
...
function auditSource(ctx, event) {
 ...
 */
export async function applyProvision(deps, q, tenantId, conn, event, existing) {
    ...
    if (existing) {
        ...
        const revoked = Boolean(lic.rowCount) && lic.rows[0].status === "revoked";
        await writeAudit(q, {
            actor: WEBHOOK_ACTOR,
    ...
    if (!extId || !mapping)
        return { applied: false, subscriptionId: null, licenseId: null, reason: "unmapped_event" };
    if (!deps.signer)
        throw new BillingError("signer_unavailable", 503, "no signer is configured for provisioning");
    const customerId = await resolveCustomer(q, conn.provider, event.externalCustomerId);
    if (!customerId)
        return { applied: false, subscriptionId: null, licenseId: null, reason: "unmapped_event" };
    const license = await issueLicense(deps.pool, deps.signer, tenantId, WEBHOOK_ACTOR, {
...
async function lockLicense(q, licenseId) {
    const r = await q("SELECT status, plan_id FROM license WHERE id = $1 FOR UPDATE", [licenseId]);
    if (!r.rowCount)
        throw new BillingError("license_not_found", 404, "the linked license no longer exists");
    const row = r.rows[0];
 ...
 */
export async function applyRenew(deps, q, tenantId, sub, event, ctx = {}) {
    const actor = ctx.actor ?? WEBHOOK_ACTOR;
    const lic = await lockLicense(q, sub.licenseId);
    if (lic.status === "revoked")
        return; // terminal — never resurrected (FR-010)
    ...
    if (eff) {
        ...
        await q(`UPDATE license
          SET entitlements = $2::jsonb,
              max_activations = $3,
              expires_at = CASE WHEN $4::double precision IS NULL THEN expires_at ELSE to_timestamp($4) END,
              updated_at = now()
        WHERE id = $1`, [sub.licenseId, JSON.stringify(entMap), eff.maxActivations, event.periodEndUnix ?? null]);
    }
 ...
 */
export async function applyGrace(deps, q, _tenantId, conn, sub, event, action, now = new Date(), ctx = {}) {
    ...
    if (lic.status === "revoked")
        return;
    const graceSeconds = resolveGraceSeconds(deps.config, { defaultGraceSeconds: conn.defaultGraceSeconds, graceOverrides: conn.graceOverrides }, event.planKey);
 ...
 */
export async function applyRevoke(deps, q, tenantId, sub, event, ctx = {}) {
    const actor = ctx.actor ?? WEBHOOK_ACTOR;
    const lic = await lockLicense(q, sub.licenseId);
