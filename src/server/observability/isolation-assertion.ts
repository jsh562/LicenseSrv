...
export function getIsolationViolationCounter(): Counter {
  return tenantIsolationViolationTotal;
}

/** Minimal structured logger the security event is emitted through (pino / Fastify `app.log` satisfy it). */
...
function defaultSecurityLogger(): SecurityEventLogger {
...
let securityLogger: SecurityEventLogger = defaultSecurityLogger();

/** Point the security-event log at a specific sink (tests / bootstrap). `undefined` restores the default. */
export function setSecurityLogger(logger: SecurityEventLogger | undefined): void {
  securityLogger = logger ?? defaultSecurityLogger();
}

/**
 * A detected tenant-isolation violation — the payload for the security signal. Sourced either from the
 * `withTenant()` assertion (`assertion`) or the synthetic canary (`canary`). Identity fields are UUIDs /
 * correlation ids (not secrets), so they are safe to log; they are deliberately NOT metric labels (OR-008).
 */
export interface IsolationViolationEvent {
  /** Server-generated request id, when the violation occurred inside a request; null for the canary. */
  readonly requestId: string | null;
  /** The authenticated principal's tenant (from the request context / canary's scoped tenant). */
  readonly authenticatedTenant: string | null;
  /** The tenant the transaction attempted to scope to (the `app.current_tenant` GUC / canary target). */
  readonly attemptedTenant: string;
  /** Where the violation was detected (assertion choke point or canary probe). */
  readonly location: string;
  /** Origin of the signal — a live request assertion or the synthetic canary probe. */
  readonly source: "assertion" | "canary";
  /**
   * The enforcement outcome: `blocked` when RLS still blocked the access (assertion path — the mismatch
   * was detected but no data leaked); `not_blocked` when the canary observed a genuine cross-tenant leak.
   */
  readonly outcome: "blocked" | "not_blocked";
}
 ...
 */
export function recordIsolationViolation(event: IsolationViolationEvent): void {
 ...
 */
export function assertTenantMatch(gucTenantId: string): void {
  try {
