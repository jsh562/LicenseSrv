 ...
 */
export function resolveSampleRatio(raw: string | undefined): number {
  if (raw === undefined || raw.trim() === "") return DEFAULT_TRACE_SAMPLE_RATIO;
  const parsed = Number(raw);
  if (!Number.isFinite(parsed)) return DEFAULT_TRACE_SAMPLE_RATIO;
 ...
 */
export const PG_INSTRUMENTATION_CONFIG = { enhancedDatabaseReporting: false } as const;
 ...
 */
export const SENSITIVE_SPAN_ATTRIBUTE_KEYS: readonly string[] = [
  "db.statement",
  "db.query.text",
...
/**
 * The ONLY attributes a signer span may carry (OR-020): availability, latency, and a coarse outcome.
 * Signing-key id/bytes and the signing payload/input MUST NEVER appear — {@link safeSignerAttributes}
 * is an allowlist by construction, so even a loosely-typed caller cannot attach key material.
 */
export interface SignerSpanAttributes {
  /** Coarse outcome bucket (e.g. `success` | `error` | `unavailable`). Never key material. */
  outcome?: string;
  /** Signer availability at call time. */
  available?: boolean;
  /** Measured signer call latency in milliseconds. */
  latencyMs?: number;
}
 ...
 */
export function safeSignerAttributes(attrs?: SignerSpanAttributes): Attributes {
  const out: Attributes = {};
  if (!attrs) return out;
  if (typeof attrs.outcome === "string") out["signer.outcome"] = attrs.outcome;
  if (typeof attrs.available === "boolean") out["signer.available"] = attrs.available;
 ...
 */
export function redactedSignerException(err: unknown): Exception {
 ...
 */
export async function withSignerSpan<T>(fn: () => Promise<T>, attrs?: SignerSpanAttributes): Promise<T> {
...
function logTracingError(message: string, err: unknown): void {
  try {
 ...
 */
export function startTracing(env: NodeJS.ProcessEnv = process.env): void {
  if (started) return;
