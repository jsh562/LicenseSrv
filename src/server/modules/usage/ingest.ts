...
import { validateDimensions, validateQuantity, type Aggregation } from "./dimension-schema.js";
import type { UsageDeps } from "./index.js";
import { classifyAcceptance } from "./rollup.js";
import type { AppendEvent } from "./usage-repo.js";

/**
 * The PER-EVENT rejection vocabulary (contract `PerEventRejectionCode`) reported inside the 200/202 ingest
 * summary — DISTINCT from the whole-request HTTP `Error.code` set. Ordered here by the FR-021 precedence.
 */
export type PerEventRejectionCode =
  | "not_found"
  | "not_metered"
  | "archived"
  | "license_inactive"
...
/** One rejected event in the per-batch summary: its zero-based `index` + a per-event `{ code, message }`. */
export interface RejectedEvent {
  index: number;
  code: PerEventRejectionCode;
  message: string;
}

/**
 * The per-batch ingest summary (contract `IngestSummary`, FR-007). `accepted` + `duplicate` + `rejected.length`
 * equals the number of events in the request: `accepted` = new events accrued, `duplicate` = same-key no-ops,
 * `rejected[]` = per-event refusals (ordered by ascending `index` for determinism).
 */
...
/** A live license snapshot the FR-021 precedence reacts to (E008): its lifecycle status + optional expiry. */
interface LicenseState {
  status: string;
  expiresAt: Date | null;
}

/** A live metered-entitlement snapshot (E007, expand-only metered kind) the precedence reacts to. */
...
function isLicenseActive(lic: LicenseState, now: Date): boolean {
...
async function readLicense(q: TxQuery, id: string): Promise<LicenseState | null> {
  const r = await q("SELECT status, expires_at FROM license WHERE id = $1", [id]);
  if (!r.rowCount) return null;
  const row = r.rows[0] as { status: string; expires_at: Date | null };
...
async function readEntitlement(q: TxQuery, id: string): Promise<EntitlementState | null> {
  const r = await q("SELECT type, status, aggregation FROM entitlement WHERE id = $1", [id]);
  if (!r.rowCount) return null;
  const row = r.rows[0] as { type: string; status: string; aggregation: Aggregation | null };
...
export async function ingestBatch(
  ...
  return withTenant(pool, tenantId, async (q): Promise<IngestSummary> => {
    ...
    const licenseCache = new Map<string, LicenseState | null>();
    const entitlementCache = new Map<string, EntitlementState | null>();
    const getLicense = async (id: string): Promise<LicenseState | null> => {
      if (!licenseCache.has(id)) licenseCache.set(id, await readLicense(q, id));
      return licenseCache.get(id) ?? null;
    };
    const getEntitlement = async (id: string): Promise<EntitlementState | null> => {
      if (!entitlementCache.has(id)) entitlementCache.set(id, await readEntitlement(q, id));
      return entitlementCache.get(id) ?? null;
    };

    const eligible: AppendEvent[] = [];
    const eligibleIndex: number[] = [];
    const reject = (index: number, code: PerEventRejectionCode, message: string): void => {
    ...
    for (const o of outcomes) {
      if (o.outcome === "accepted") accepted++;
      else duplicate++;
    }
