...
async function licenseExists(q: TxQuery, licenseId: string): Promise<boolean> {
...
async function auditRateLimited(pool: pg.Pool, secret: string, req: FastifyRequest): Promise<void> {
  ...
  await withTenant(pool, ctx.tenantId, (q) =>
    recordSecurityEvent(q, { actor: "usage-api", action: "usage.rate_limited", target: `${req.method} ${req.url}` }),
  );
}
 ...
 */
async function requireIngestScope(pool: pg.Pool, req: FastifyRequest, reply: FastifyReply): Promise<boolean> {
  if (!req.tenant) {
    await err(reply, 401, "unauthorized", "missing tenant context");
    return false;
  }
  if (!req.tenant.scopes.includes("usage.ingest")) {
    await withTenant(pool, req.tenant.tenantId, (q) =>
      recordSecurityEvent(q, { actor: "usage-api", action: "usage.scope_denied", target: "usage.ingest" }),
    ).catch(() => undefined);
 ...
 */
export function registerUsageRoutes(app: FastifyInstance, deps: UsageDeps, apiKeySecret: string): void {
  ...
  void app.register(async (scope) => {
    ...
    scope.post("/v1/usage", rl, async (req, reply): Promise<FastifyReply> => {
      if (!(await requireIngestScope(pool, req, reply))) return reply;
    ...
    scope.get("/v1/licenses/:licenseId/usage", rl, async (req, reply): Promise<FastifyReply> => {
      if (!(await requireIngestScope(pool, req, reply))) return reply;
      const tenantId = req.tenant!.tenantId;

      const licenseId = (req.params as { licenseId: string }).licenseId;
      if (!UUID_RE.test(licenseId)) return err(reply, 404, "not_found", "no such license in this tenant", { licenseId });
      ...
      if (to.getTime() <= from.getTime()) return validation(reply, "to must be after from", { field: "to" });
      ...
      const result = await withTenant(pool, tenantId, async (q) => {
        if (!(await licenseExists(q, licenseId))) return null;
      ...
      if (!result) return err(reply, 404, "not_found", "no such license in this tenant", { licenseId });
  ...
  void app.register(async (adminScope) => {
    ...
    adminScope.get("/admin/licenses/:licenseId/usage", viewer, async (req, reply): Promise<FastifyReply> => {
      const licenseId = (req.params as { licenseId: string }).licenseId;
      if (!UUID_RE.test(licenseId)) return err(reply, 404, "not_found", "no such license in this tenant", { licenseId });
