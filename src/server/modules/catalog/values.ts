...
import { assertValueMatchesType, CatalogError, type EntitlementType } from "./validation.js";
...
function toValue(type: EntitlementType, boolValue: boolean | null, intValue: number | null): boolean | number {
...
export async function listPlanEntitlements(pool: pg.Pool, tenantId: string, planId: string): Promise<PlanEntitlementValue[]> {
  return withTenant(pool, tenantId, async (q) => {
    const plan = await q("SELECT 1 FROM plan WHERE id = $1", [planId]);
    if (!plan.rowCount) throw new CatalogError("not_found", 404, "unknown plan");
    const r = await q(
      `SELECT pe.entitlement_id, e.key, e.type, pe.bool_value, pe.int_value
    ...
    return (r.rows as { entitlement_id: string; key: string; type: EntitlementType; bool_value: boolean | null; int_value: number | null }[]).map(
      (row) => ({ entitlementId: row.entitlement_id, key: row.key, type: row.type, value: toValue(row.type, row.bool_value, row.int_value) }),
    );
  });
}
 ...
 */
export async function setPlanEntitlementValue(
  pool: pg.Pool,
  tenantId: string,
  actor: string,
  planId: string,
  entitlementId: string,
  value: unknown,
): Promise<PlanEntitlementValue> {
  return withTenant(pool, tenantId, async (q): Promise<PlanEntitlementValue> => {
    const plan = await q("SELECT status FROM plan WHERE id = $1", [planId]);
    if (!plan.rowCount) throw new CatalogError("not_found", 404, "unknown plan");
    if ((plan.rows[0] as { status: string }).status !== "active") {
    ...
    if (!ent.rowCount) throw new CatalogError("not_found", 404, "unknown entitlement");
    const { key, type, status } = ent.rows[0] as { key: string; type: EntitlementType; status: string };
    if (status !== "active") throw new CatalogError("archived", 409, "cannot set a value for an archived entitlement");

    const { boolValue, intValue } = assertValueMatchesType(type, value);

    await q(
      `INSERT INTO plan_entitlement (id, tenant_id, plan_id, entitlement_id, bool_value, int_value)
       VALUES ($1, current_setting('app.current_tenant')::uuid, $2, $3, $4, $5)
       ON CONFLICT (tenant_id, plan_id, entitlement_id)
       DO UPDATE SET bool_value = EXCLUDED.bool_value, int_value = EXCLUDED.int_value, updated_at = now()`,
      [randomUUID(), planId, entitlementId, boolValue, intValue],
    );
...
export async function removePlanEntitlementValue(
  pool: pg.Pool,
  tenantId: string,
  actor: string,
  planId: string,
  ...
  return withTenant(pool, tenantId, async (q) => {
    ...
    if (!r.rowCount) throw new CatalogError("not_found", 404, "no such value on this plan");
