...
/**
 * The platform-default branding floor (the lowest precedence tier). Keyed by the contract `BrandingFieldName`
 * camelCase names the per-field resolver walks (FR-007) — presentation fields only; `emailSenderAddress` /
 * `customDomain` are NOT platform-defaulted (they take effect only via an `active` `domain_binding`, FR-013).
 */
export interface PlatformBranding {
  /** Platform-default logo reference (contract `logoUrl` → column `logo_ref`). */
  logoUrl: string;
  /** Platform-default primary color (contract `primaryColor` → column `color_primary`). */
  primaryColor: string;
  /** Platform-default secondary color (contract `secondaryColor` → column `color_secondary`). */
  secondaryColor: string;
  /** Platform-default product name (contract `productName` → column `product_name`). */
  productName: string;
  /** Platform-default support URL (contract `supportUrl` → column `support_url`). */
  supportUrl: string;
  /** Platform-default help URL (contract `helpUrl` → column `help_url`). */
  helpUrl: string;
}

/** The resolved deployment-wide reseller config. Times are in seconds as the field name says; quota is a count. */
export interface ResellerConfig {
  /** Default hard sub-tenant cap a newly-onboarded reseller receives; a non-negative count, 0 allowed (FR-003/010). */
  defaultSubTenantQuota: number;
  /** Offboarding grace window in seconds; `graceEndsAt = offboarding_started_at + this` (FR-012). */
  offboardingGraceSecs: number;
  /** The non-white-labelable trust-signal set — always authoritative, never sourced from branding (FR-008). */
  trustSignals: readonly string[];
  /** The platform-default branding floor backing the lowest precedence tier (FR-007). */
...
export const DEFAULT_TRUST_SIGNALS: readonly string[] = [
  "revocation",
  "tamper",
  "signing_identity",
  "audit",
...
function posIntEnv(raw: string | undefined, dflt: number): number {
  const n = Number(raw);
  return Number.isFinite(n) && n > 0 ? Math.floor(n) : dflt;
}

/** Resolve a string env value, falling back to `dflt` when missing/empty (an empty override is treated as unset). */
function strEnv(raw: string | undefined, dflt: string): string {
 ...
 */
export function parseTrustSignals(raw: string | undefined): readonly string[] {
  if (raw === undefined) return DEFAULT_TRUST_SIGNALS;
 ...
 */
export function loadResellerConfig(env: NodeJS.ProcessEnv = process.env): ResellerConfig {
  return {
