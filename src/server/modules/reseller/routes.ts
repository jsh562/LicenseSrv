// The reseller + operator admin REST surface (E018, US1; FR-002/003/004/017). ADMIN-PLANE, console-only — the
// shared E005 console session cookie + RBAC (`owner>admin>viewer`, viewer reads / admin mutates) + double-submit
// CSRF on every mutation, mirroring the E006/E016/E017 admin routes. camelCase bodies; project
// `{code,message,details?}` errors; a thrown ResellerError maps to its HTTP status + machine code.
//
// THREE ACTOR PLANES, ONE ISOLATION MODEL (AD-001/002, HINT-001/002):
//   * RESELLER plane — a reseller-admin (an admin/owner of a tenant that IS a reseller) acting over its WHOLE
//     subtree. `requireResellerPlane` runs AFTER the console `requireRole` (session + role + CSRF): it asserts
//     the session tenant has a `reseller` row (reseller-admin = admin/owner of a reseller tenant, AD-001) and
//     fails CLOSED — a non-reseller session is denied `403 forbidden` + a security-event audit (T017,
//     [COMPLETES FR-002]). Authority over a sub-tenant is the subtree-membership GATE (`assertSubtreeMembership`)
//     — an out-of-subtree id resolves `404 not_found` (NEVER 403 — 403 leaks existence) + a security event
//     (T018, [COMPLETES FR-004], HINT-002).
//   * OPERATOR plane — the vendor/platform operator (the actor above all resellers). `requireOperatorPlane`
//     fails closed for a reseller-admin (or a sub-tenant admin) attempting an operator-reserved action → `403
//     forbidden` + a security event (US1-AS4). The full operator lifecycle (re-quota/suspend/offboard/move
//     business logic) lands in US4; US1 wires the move seam so the delegated-vs-operator boundary is enforced +
//     tested now.
//
// The sub-tenant projection is METADATA-ONLY (FR-017, [COMPLETES FR-017] via `toSubTenantWire`, T020): a reseller
// sees ONLY a sub-tenant's id/display-name/status/read-only-cascade/created-at — NO license, usage, or activation
// data (nor does the `tenant`/`SubTenantRow` projection carry any). Performs NO cryptography (Principle I).
//
// ─── SECURITY GUARANTEE (T047, FINALIZED — every mutation, all three planes) ─────────────────────────────────
// EVERY state-changing route below is registered with an `*Admin` preHandler pair `[requireRole(pool,"admin"),
// <plane>]` (reseller mutations → `resellerAdmin`; operator mutations → `operatorAdmin`; a tenant's own-branding
// mutation → `ownAdmin`). `requireRole` (E005) fails CLOSED and, in ONE place, enforces all THREE of: (1) an
// authenticated console SESSION (else 401), (2) the correct minRole in `owner>admin>viewer` — `admin` for every
// mutation, `viewer` for every read (else 403), and (3) DOUBLE-SUBMIT CSRF on every non-GET/HEAD/OPTIONS request
// (cookie `admin_csrf` ↔ header `X-CSRF-Token`, else 403). It then runs the plane gate. So the invariant holds
// uniformly: NO reachable mutation lacks session + RBAC(admin) + CSRF, and every read requires >= viewer.
//
// EVERY DENIAL IS AUDITED AS A SECURITY EVENT (fail-closed, tenant-scoped, append-only):
//   * CSRF-missing/mismatch  → `authz.denied` (…"(csrf)") — recorded by `requireRole` before any effect.
//   * RBAC (no role / < admin) → `authz.denied` — recorded by `requireRole`.
//   * PLANE (reseller-admin/sub-tenant escalating to an operator action, or a non-reseller on the reseller plane)
//        → `operator.plane.denied` (DUAL-IDENTITY via `recordResellerSecurityEvent`) / `reseller.plane.denied`
//        — recorded by `requireOperatorPlane` / `requireResellerPlane` below, then 403.
//   * SUBTREE (out-of-subtree / IDOR sub-tenant reference) → `reseller.subtree.denied` (DUAL-IDENTITY) — recorded
//        inside `assertSubtreeMembershipAudited` (gate.ts, T031), resolving 404 with NO existence disclosure
//        (never 403 — 403 leaks existence, HINT-002).
// A 403 is thus reserved for RBAC/plane/CSRF; an out-of-scope id is ALWAYS 404 + a security event. No mutation
// path can bypass this pairing — the boundary is the preHandler registration, not the handler body.
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────
...
import type { ResellerRow, SubTenantRow } from "./reseller-repo.js";
import type { DomainBinding } from "./verify.js";

declare module "fastify" {
  interface FastifyRequest {
    /** The acting reseller's own row — set by `requireResellerPlane` after asserting the session tenant is a reseller. */
...
async function guard(reply: FastifyReply, fn: () => Promise<FastifyReply>): Promise<FastifyReply> {
  try {
    ...
    if (e instanceof ResellerError) return err(reply, e.status, e.code, e.message, e.details);
    throw e;
  }
}
 ...
 */
async function resolveActiveBoundFields(
  deps: ResellerDeps,
  tenantId: string,
  ...
  return new Set<BrandingFieldName>([...own, ...reseller]);
}

/** The DomainBinding wire projection (contract `DomainBinding`) — dates serialized ISO-8601; challenge PUBLIC. */
function toDomainBindingWire(b: DomainBinding): Record<string, unknown> {
  return {
 ...
 */
function toSubTenantWire(
  row: SubTenantRow,
  readOnly: boolean,
  opts: { includeResellerId?: boolean } = {},
  ...
  if (opts.includeResellerId) wire.resellerId = row.parentResellerId;
  return wire;
}
 ...
 */
function requireResellerPlane(deps: ResellerDeps) {
  return async function preHandler(req: FastifyRequest, reply: FastifyReply): Promise<void> {
    ...
    if (!reseller) {
      await withTenant(deps.pool, admin.tenantId, (q) =>
 ...
 */
function requireOperatorPlane(deps: ResellerDeps) {
  return async function preHandler(req: FastifyRequest, reply: FastifyReply): Promise<void> {
    const admin = req.admin;
    if (!admin) {
      await reply.code(401).send({ code: "unauthorized", message: "authentication required" });
 ...
 */
export function registerResellerRoutes(app: FastifyInstance, deps: ResellerDeps): void {
  ...
  app.get("/admin/reseller/sub-tenants", resellerViewer, async (req, reply): Promise<FastifyReply> => {
    ...
    return guard(reply, async () => {
      ...
      const page = (truncated ? rows.slice(0, LIST_CAP) : rows).sort(
  ...
  app.post("/admin/reseller/sub-tenants", resellerAdmin, async (req, reply): Promise<FastifyReply> => {
    ...
    const admin = req.admin!;
    return guard(reply, async () => {
      const row = await provisionSubTenant(
        { pool, repo },
        {
  ...
  app.get("/admin/reseller/sub-tenants/:subTenantId", resellerViewer, async (req, reply): Promise<FastifyReply> => {
    const subTenantId = (req.params as { subTenantId: string }).subTenantId;
    const admin = req.admin!;
    const reseller = req.reseller!;
  ...
  app.post(
    "/admin/operator/sub-tenants/:subTenantId/move",
    operatorAdmin,
    async (req, reply): Promise<FastifyReply> => {
      ...
      const admin = req.admin!;
      return guard(reply, async () => {
        const moved = await moveSubTenant(
          { pool, repo, branding, config },
          { subTenantId, destination: b.data.destination, actorUserId: admin.userId },
        );
  ...
  app.post(
    "/admin/operator/resellers/:resellerId/suspend",
    operatorAdmin,
    async (req, reply): Promise<FastifyReply> => {
      const resellerId = (req.params as { resellerId: string }).resellerId;
  ...
  app.post(
    "/admin/operator/resellers/:resellerId/reinstate",
    operatorAdmin,
    async (req, reply): Promise<FastifyReply> => {
      const resellerId = (req.params as { resellerId: string }).resellerId;
      if (!UUID_RE.test(resellerId)) {
  ...
  app.post(
    "/admin/operator/resellers/:resellerId/offboard",
    operatorAdmin,
    async (req, reply): Promise<FastifyReply> => {
      const resellerId = (req.params as { resellerId: string }).resellerId;
      if (!UUID_RE.test(resellerId)) {
  ...
  app.post("/admin/operator/resellers", operatorAdmin, async (req, reply): Promise<FastifyReply> => {
  ...
  app.get("/admin/operator/resellers", operatorViewer, async (req, reply): Promise<FastifyReply> => {
  ...
  app.get("/admin/operator/resellers/:resellerId", operatorViewer, async (req, reply): Promise<FastifyReply> => {
    const resellerId = (req.params as { resellerId: string }).resellerId;
    if (!UUID_RE.test(resellerId)) {
      return err(reply, 404, "not_found", "reseller not found", { resellerId });
    }
    return guard(reply, async () => {
      ...
      if (!m) return err(reply, 404, "not_found", "reseller not found", { resellerId });
  ...
  // admin + CSRF + audited.
  app.patch(
    "/admin/operator/resellers/:resellerId/quota",
    operatorAdmin,
    async (req, reply): Promise<FastifyReply> => {
      const resellerId = (req.params as { resellerId: string }).resellerId;
      if (!UUID_RE.test(resellerId)) {
      ...
      return guard(reply, async () => {
        ...
        if (!existing) return err(reply, 404, "not_found", "reseller not found", { resellerId });
        ...
        if (!updated) return err(reply, 404, "not_found", "reseller not found", { resellerId });
        await withTenant(pool, resellerId, (q) =>
          writeAudit(q, {
            actor: admin.userId,
  ...
  app.get("/admin/reseller/branding", resellerViewer, async (req, reply): Promise<FastifyReply> => {
    const reseller = req.reseller!;
    return guard(reply, async () => {
      const profile = await branding.getProfile(reseller.tenantId);
      const layer: BrandingLayer = profile ?? { fields: {}, lockedFields: [] };
  ...
  app.put("/admin/reseller/branding", resellerAdmin, async (req, reply): Promise<FastifyReply> => {
    ...
    return guard(reply, async () => {
      ...
      await withTenant(pool, reseller.tenantId, (q) =>
        writeAudit(q, {
          actor: admin.userId,
  ...
  app.get("/admin/branding", ownViewer, async (req, reply): Promise<FastifyReply> => {
    const admin = req.admin!;
    return guard(reply, async () => {
      ...
      const ownLayer: BrandingLayer = own ?? { fields: {}, lockedFields: [] };
  ...
  app.put("/admin/branding", ownAdmin, async (req, reply): Promise<FastifyReply> => {
    ...
    return guard(reply, async () => {
      ...
      const parentId = await repo.getParentResellerId(admin.tenantId);
      const resellerLayer = parentId ? await branding.getProfilePrivileged(parentId) : null;
      const lockedSet = new Set<BrandingFieldName>(resellerLayer?.lockedFields ?? []);
      // A reseller-locked field is authoritative — refuse the override WITHOUT revealing the reseller (STF-001/004).
      for (const f of Object.keys(b.data.overrides) as BrandingFieldName[]) {
        if (lockedSet.has(f)) {
      ...
      await withTenant(pool, admin.tenantId, (q) =>
        writeAudit(q, { actor: admin.userId, action: "branding.set", target: admin.tenantId, after: { overrides: b.data.overrides } }),
      );
      const activeBoundFields = await resolveActiveBoundFields(deps, admin.tenantId, parentId);
      const resolved = resolveBranding({
        subTenant: profile,
  ...
  function assertResellerActive(reseller: ResellerRow): void {
    if (reseller.status === "suspended") {
  ...
  app.get("/admin/reseller/domains", resellerViewer, async (req, reply): Promise<FastifyReply> => {
    const reseller = req.reseller!;
    return guard(reply, async () => {
  ...
  app.post("/admin/reseller/domains", resellerAdmin, async (req, reply): Promise<FastifyReply> => {
  ...
  app.get("/admin/reseller/domains/:bindingId", resellerViewer, async (req, reply): Promise<FastifyReply> => {
    const bindingId = (req.params as { bindingId: string }).bindingId;
    const reseller = req.reseller!;
    if (!UUID_RE.test(bindingId)) return err(reply, 404, "not_found", "binding not found", { bindingId });
    return guard(reply, async () => {
      const binding = await deps.verifier.get(reseller.tenantId, bindingId);
      if (!binding) return err(reply, 404, "not_found", "binding not found", { bindingId });
  ...
  app.post("/admin/reseller/domains/:bindingId/verify", resellerAdmin, async (req, reply): Promise<FastifyReply> => {
    const bindingId = (req.params as { bindingId: string }).bindingId;
    const reseller = req.reseller!;
    const admin = req.admin!;
    if (!UUID_RE.test(bindingId)) return err(reply, 404, "not_found", "binding not found", { bindingId });
  ...
  app.post("/admin/reseller/domains/:bindingId/activate", resellerAdmin, async (req, reply): Promise<FastifyReply> => {
    const bindingId = (req.params as { bindingId: string }).bindingId;
    const reseller = req.reseller!;
    const admin = req.admin!;
    if (!UUID_RE.test(bindingId)) return err(reply, 404, "not_found", "binding not found", { bindingId });
