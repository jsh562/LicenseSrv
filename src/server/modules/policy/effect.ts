...
/** The closed set of effect kinds a rule may carry (AD-002, FR-003). */
export type EffectKind = "adjust_limit" | "toggle_boolean" | "select_tier";

/** A closed typed effect descriptor a rule returns; the trusted applier validates + bounds it (never mutation). */
export interface PolicyEffect {
  kind: EffectKind;
  /** The entitlement decision key the effect adjusts (e.g. `api_calls`). */
  target: string;
  /** The requested value (a static literal): a number (adjust_limit or select_tier) or boolean (toggle_boolean). */
  value: unknown;
}

/** The E007 authored per-entitlement bound the applier clamps to (from the target entitlement, AD-003). */
export interface EntitlementBounds {
  /** Authored maximum for an adjust_limit effect (≥ base plan value); NULL/undefined → no rule-raise allowed. */
  ruleMax?: number | null;
  /** Whether a toggle_boolean effect may flip this boolean entitlement (the plan marks it rule-eligible). */
  ruleEligible?: boolean;
  /** The plan-defined numeric select_tier options a rule may select among; NULL/undefined → no tiers. */
  ruleTiers?: readonly unknown[] | null;
  /** Optional absolute per-entitlement ceiling (config FR-021); the clamp uses min(ruleMax, absoluteMax). */
  absoluteMax?: number | null;
}

/** The outcome of applying an effect: the bounded value to enforce, or a fail-closed refusal with a reason. */
export type ApplyEffectResult =
  | { applied: true; kind: EffectKind; target: string; value: number | boolean; clamped: boolean }
  | { applied: false; kind: EffectKind; target: string; reason: string };
 ...
 */
function resolveCeiling(bounds: EntitlementBounds): number | null {
  if (typeof bounds.ruleMax !== "number" || !Number.isFinite(bounds.ruleMax)) return null;
...
function tierAllowed(value: number, tiers: readonly unknown[] | null | undefined): boolean {
 ...
 */
export function applyEffect(effect: PolicyEffect, bounds: EntitlementBounds): ApplyEffectResult {
  ...
  switch (kind) {
    ...
    default: {
      ...
      const unknownKind = kind as string;
      return { applied: false, kind: unknownKind as EffectKind, target, reason: "unknown_kind" };
    }
  }
}
