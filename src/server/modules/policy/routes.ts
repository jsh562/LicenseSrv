...
import type { PolicyDeps } from "./index.js";
import { PolicyError } from "./index.js";
import {
  buildDecisionContext,
  buildSuppliedContext,
  canonicalContextHash,
  ContextError,
  type BuildContextOptions,
} from "./context.js";
import type { EvaluateConditionOptions } from "./condition.js";
import { applyEffect, type EntitlementBounds, type PolicyEffect } from "./effect.js";
import { resolveEntitlementDecision, type CandidateRule } from "./evaluate.js";
import type { PolicyRuleRow, RuleStatus } from "./rule-repo.js";
import {
  assertRuleSetWithinCaps,
  lintRuleConflicts,
  validateRule,
  type RuleLintWarning,
...
async function guard(reply: FastifyReply, fn: () => Promise<FastifyReply>): Promise<FastifyReply> {
  try {
    ...
    if (e instanceof PolicyError) return err(reply, e.status, e.code, e.message, e.details);
    throw e;
  }
}
 ...
 */
const PERMITTED_TRANSITIONS: Record<RuleStatus, readonly RuleStatus[]> = {
...
function toVersion(row: PolicyRuleRow): Record<string, unknown> {
  return {
    ruleKey: row.ruleKey,
    version: row.version,
 ...
 */
function liveRuleLint(
  liveRules: readonly PolicyRuleRow[],
  candidate: { priority: number; condition: unknown; ruleKey?: string },
): RuleLintWarning[] {
  return lintRuleConflicts(
    candidate,
    liveRules.map((r) => ({ ruleKey: r.ruleKey, version: r.version, priority: r.priority, condition: r.condition })),
  );
}

/** The create/edit response: the persisted immutable version PLUS any non-blocking author-time lint warnings. */
function withWarnings(row: PolicyRuleRow, warnings: RuleLintWarning[]): Record<string, unknown> {
  return { ...toVersion(row), warnings };
}

/** A latest-version summary wire projection (contract `PolicyRuleSummary`). */
function toSummary(row: PolicyRuleRow): Record<string, unknown> {
  const effect = row.effect as { kind?: unknown } | null;
  return {
    ruleKey: row.ruleKey,
 ...
 */
export function registerPolicyRoutes(app: FastifyInstance, deps: PolicyDeps): void {
  ...
  const validateOpts: ValidateRuleOptions = {
  ...
  function boundsOf(ent: { ruleMax: number | null; ruleEligible: boolean; ruleTiers: unknown[] | null }): EntitlementBounds {
    return {
      ruleMax: ent.ruleMax,
  ...
  app.post("/admin/policy/rules", admin, async (req, reply): Promise<FastifyReply> => {
    ...
    return guard(reply, async () => {
      ...
      if (!ent) return err(reply, 404, "not_found", "unknown entitlement", { entitlementId: b.data.targetEntitlementId });
      ...
      const status: RuleStatus = b.data.status ?? "preview";
  ...
  app.get("/admin/policy/rules", viewer, async (req, reply): Promise<FastifyReply> => {
    ...
    const tenantId = req.admin!.tenantId;
    const rows = await withTenant(pool, tenantId, (t) =>
      repo.listLatestRules(t, { entitlementId: q.data.entitlementId, status: q.data.status, limit: LIST_CAP + 1 }),
    );
  ...
  app.get("/admin/policy/rules/:ruleKey", viewer, async (req, reply): Promise<FastifyReply> => {
    const ruleKey = (req.params as { ruleKey: string }).ruleKey;
    if (!UUID_RE.test(ruleKey)) return err(reply, 404, "not_found", "no such rule in this tenant", { ruleKey });
    const tenantId = req.admin!.tenantId;
    const versions = await withTenant(pool, tenantId, (t) => repo.getVersions(t, ruleKey));
    if (versions.length === 0) return err(reply, 404, "not_found", "no such rule in this tenant", { ruleKey });
  ...
  app.patch("/admin/policy/rules/:ruleKey", admin, async (req, reply): Promise<FastifyReply> => {
    const ruleKey = (req.params as { ruleKey: string }).ruleKey;
    if (!UUID_RE.test(ruleKey)) return err(reply, 404, "not_found", "no such rule in this tenant", { ruleKey });
    ...
    return guard(reply, async () => {
      ...
      if (!loaded) return err(reply, 404, "not_found", "no such rule in this tenant", { ruleKey });

      const ent = await entitlementRead(pool, tenantId, loaded.entitlementId);
      if (!ent) return err(reply, 404, "not_found", "no such rule in this tenant", { ruleKey });
      ...
      const newStatus: RuleStatus = loaded.live ? loaded.live.status : "disabled";
      const persisted = await withTenant(pool, tenantId, async (t) => {
        ...
        const warnings = liveRuleLint(
          await repo.selectLiveRulesForEntitlement(t, loaded.entitlementId, config.maxRulesPerIssuance + 1),
          { priority: b.data.priority, condition: b.data.condition, ruleKey },
        );
        if (loaded.live) await repo.updateStatus(t, { ruleKey, version: loaded.live.version, status: "disabled" });
  ...
  app.post("/admin/policy/rules/:ruleKey/status", admin, async (req, reply): Promise<FastifyReply> => {
    const ruleKey = (req.params as { ruleKey: string }).ruleKey;
    if (!UUID_RE.test(ruleKey)) return err(reply, 404, "not_found", "no such rule in this tenant", { ruleKey });
    ...
    return guard(reply, async () => {
      const updated = await withTenant(pool, tenantId, async (t) => {
        const versions = await repo.getVersions(t, ruleKey);
        if (versions.length === 0) return null; // unknown / cross-tenant → 404 (RLS, FR-015)
        const head = versions[0]!;
      ...
      if (!updated) return err(reply, 404, "not_found", "no such rule in this tenant", { ruleKey });
   ...
   */
  async function assembleRealContext(
    tenantId: string,
    licenseId: string,
    ent: { key: string; type: string; ruleMax: number | null; ruleEligible: boolean; ruleTiers: unknown[] | null },
  ): Promise<{ context: ReturnType<typeof buildDecisionContext>; decisionTimestampMs: number; baseValue: number | boolean | null; planId: string } | null> {
    const licenseRow = await withTenant(pool, tenantId, async (t) => {
      ...
      return (r.rowCount ?? 0) === 0 ? null : (r.rows[0] as { plan_id: string; status: string; expires_at: Date | null; entitlements: Record<string, unknown> });
    });
    ...
    const baseValue: number | boolean | null = effEnt
      ? effEnt.value
  ...
  app.post("/admin/policy/rules/:ruleKey/dry-run", admin, async (req, reply): Promise<FastifyReply> => {
    const ruleKey = (req.params as { ruleKey: string }).ruleKey;
    if (!UUID_RE.test(ruleKey)) return err(reply, 404, "not_found", "no such rule in this tenant", { ruleKey });
    ...
    return guard(reply, async () => {
      ...
      let simCondition: unknown = baseVersion.condition;
      let simEffect: unknown = baseVersion.effect;
      let simPriority = baseVersion.priority;
      if (b.data.candidate) {
        validateRule(
          { condition: b.data.candidate.condition, effect: b.data.candidate.effect, bounds: boundsOf(ent) },
          validateOpts,
        );
      ...
      let decisionTimestampMs: number;
      let baseValue: number | boolean | null;
      let effBounds: EntitlementBounds;
      let auditLicenseId: string | null;
      let auditPlanId: string | null;
      if (hasContext) {
        let supplied;
        try {
        ...
        if (!real) return err(reply, 404, "not_found", "no such rule or license in this tenant", { licenseId: b.data.licenseId });
      ...
      const targetCand: CandidateRule = {
        id: baseVersion.id,
        ruleKey,
      ...
      const otherLive = await withTenant(pool, tenantId, (t) =>
        repo.selectLiveRulesForEntitlement(t, targetEntitlementId, config.maxRulesPerIssuance + 1),
      );
      const others: CandidateRule[] = otherLive
      ...
      const baseForResolve =
        typeof baseValue === "number" || typeof baseValue === "boolean" || typeof baseValue === "string" ? baseValue : 0;
      ...
      if (resolved.firedRule) {
        ...
        if (winner) {
          const applied = applyEffect(winner.effect as PolicyEffect, effBounds);
          if (applied.applied) {
      ...
      await withTenant(pool, tenantId, (t) =>
        repo.appendEvaluation(t, {
