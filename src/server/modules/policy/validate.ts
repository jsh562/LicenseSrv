...
import { applyEffect, type EntitlementBounds, type EffectKind, type PolicyEffect } from "./effect.js";
import { PolicyError } from "./index.js";

/**
 * The allow-listed decision-context schema a `var`/`has` path is type-checked against (FR-004, the minimization
 * boundary). Each root maps to either `"leaf"` (a terminal field, no sub-path — e.g. the injected `now`),
 * `"any"` (a dynamic sub-key space — the metered `usage.*` aggregates, `has()`-guarded at evaluation), or an
 * explicit field allow-list (a fixed set of allowed second segments). A path whose root or field is absent is
 * REFUSED (`invalid_condition`).
 */
export type ContextFieldSchema = Readonly<Record<string, "leaf" | "any" | readonly string[]>>;

/** The built-in decision-context schema (mirrors the sections context.ts assembles: E007/E008/E016 + `now`). */
export const DEFAULT_CONTEXT_SCHEMA: ContextFieldSchema = {
  now: "leaf",
...
/** Author-time resource-bound + schema options; each falls back to the documented default. */
export interface ValidateRuleOptions {
  /** Serialized condition byte cap (FR-009) → `condition_too_large`. */
  maxBytes?: number;
  /** Guarded-condition AST depth cap (FR-009) → `condition_too_large`. */
  maxDepth?: number;
  /** Guarded-condition node-count / complexity cap (FR-009) → `condition_too_large`. */
  maxComplexity?: number;
  /** Override the allow-listed context schema (defaults to {@link DEFAULT_CONTEXT_SCHEMA}). */
  contextSchema?: ContextFieldSchema;
}

/** The rule content to validate at author time: the guarded condition + typed effect + the target's bound. */
export interface ValidateRuleInput {
  /** The structured-JSON (JSONLogic-subset) condition (FR-001/002). */
  condition: unknown;
  /** The closed typed effect descriptor `{kind, target, value}` (AD-002, FR-003). */
  effect: unknown;
  /** The target entitlement's authored bound (rule_max/rule_eligible/rule_tiers + absolute cap, AD-003). */
...
interface LintState {
  ...
  readonly schema: ContextFieldSchema;
}
 ...
 * refused (`invalid_condition`).
 */
function lintFieldPath(op: string, path: string, schema: ContextFieldSchema): void {
  ...
  if (rootSchema === "any") return; // dynamic sub-key space (usage.* metered aggregates)
...
function lintVarArg(op: string, arg: unknown, state: LintState): void {
  let path: unknown;
  if (Array.isArray(arg)) {
    path = arg[0];
    if (arg.length > 1) lintNode(arg[1], state); // the has()-guard default alternative is a real node
  } else {
    path = arg;
  }
 ...
 */
function lintNode(node: unknown, state: LintState): void {
  state.ops += 1;
  if (state.ops > state.maxComplexity) {
    throw new PolicyError("condition_too_large", 400, "condition exceeds the complexity budget");
  }
  if (node === null || typeof node !== "object") return; // literal (string/number/boolean/null)
  if (Array.isArray(node)) {
    for (const child of node) lintNode(child, state);
    return;
  }
  ...
  if (op === "var" || op === "has") {
    ...
    for (const child of asArray(arg)) lintNode(child, state);
  }
  state.depth -= 1;
}

/** Validate the structured-JSON condition: object shape → byte cap → operator/field/depth/complexity walk. */
function validateCondition(condition: unknown, opts: ValidateRuleOptions): void {
 ...
 */
function validateEffect(effect: unknown, bounds: EntitlementBounds): void {
  ...
  const descriptor: PolicyEffect = { kind: kind as EffectKind, target: e.target, value: e.value };
 ...
 */
export function validateRule(input: ValidateRuleInput, opts: ValidateRuleOptions = {}): void {
  validateCondition(input.condition, opts);
  validateEffect(input.effect, input.bounds);
}

/** The tenant's CURRENT live (active|preview) rule-set sizes the FR-019 author-time size caps are checked against. */
export interface RuleSetLiveCounts {
  /** LIVE rule versions already targeting the effect's entitlement (the per-entitlement cap dimension). */
  entitlementLive: number;
  /** LIVE rule versions across the whole tenant (the per-tenant cap dimension). */
  tenantLive: number;
}

/** The configured FR-019 author-time rule-set size caps (from `PolicyConfig`). */
 ...
 */
export function assertRuleSetWithinCaps(counts: RuleSetLiveCounts, caps: RuleSetCaps): void {
  if (counts.entitlementLive >= caps.maxRulesPerEntitlement) {
    throw new PolicyError(
      "rule_set_limit_exceeded",
      400,
      "the entitlement's live rule set is at its configured maximum",
      { limit: caps.maxRulesPerEntitlement, actual: counts.entitlementLive + 1, scope: "entitlement" },
    );
  }
  if (counts.tenantLive >= caps.maxRulesPerTenant) {
    throw new PolicyError(
      "rule_set_limit_exceeded",
      400,
      "the tenant's live rule set is at its configured maximum",
      { limit: caps.maxRulesPerTenant, actual: counts.tenantLive + 1, scope: "tenant" },
    );
  }
}

// --- Author-time overlap / unreachable-rule LINT (US6, FR-006, SC-010) ---------------------------------------
//
// Highest-priority-wins (AD-005/INV-5) fires exactly ONE matching rule per entitlement, scanned in a
// DETERMINISTIC order (`priority DESC`, then the stable `(rule_key ASC, version DESC)` tiebreak). As a rule set
// grows an author can inadvertently add a rule that can NEVER win, or a same-priority peer whose precedence hinges
// only on the tiebreak. This is a NON-BLOCKING author-time lint (a WARNING, not a refusal — the rule still
// persists, FR-006/US6-AS3): it surfaces the overlap so the author can retune the priority. Pure — no DB, no eval
// of live data; it reuses the SAME sandboxed evaluator to constant-fold an UNCONDITIONAL (context-free) condition.

/** A distinct, testable author-time lint finding about rule precedence (non-blocking, FR-006/SC-010). */
export type RuleLintCode = "unreachable_rule" | "overlapping_rule";

/** One non-blocking lint finding: the new/edited rule overlaps or is shadowed by an existing live peer. */
export interface RuleLintWarning {
  code: RuleLintCode;
  message: string;
  /** The existing live rule this finding relates to (the shadowing / overlapping peer). */
  ruleKey: string;
  version: number;
  priority: number;
}

/** An existing LIVE (active|preview) rule targeting the SAME entitlement, linted against the new/edited rule. */
export interface LiveRuleForLint {
  ruleKey: string;
  version: number;
  priority: number;
  condition: unknown;
...
/** The candidate rule being authored/edited (its priority + condition; the target entitlement is the caller scope). */
export interface LintRuleInput {
  priority: number;
  condition: unknown;
  /** The candidate's own logical rule_key — excluded from the peer comparison (an EDIT re-lints against peers). */
  ruleKey?: string;
}

/** True when a condition node (or any descendant) references the decision context via `var`/`has` (data-dependent). */
function referencesContext(node: unknown): boolean {
  if (node === null || typeof node !== "object") return false;
  if (Array.isArray(node)) return node.some(referencesContext);
  const keys = Object.keys(node);
  if (keys.length === 1) {
    const op = keys[0]!;
    if (op === "var" || op === "has") return true;
  }
  return Object.values(node as Record<string, unknown>).some(referencesContext);
}
...
function isUnconditional(condition: unknown): boolean {
  if (referencesContext(condition)) return false;
  try {
 ...
 */
export function lintRuleConflicts(
  input: LintRuleInput,
  existingLive: readonly LiveRuleForLint[],
): RuleLintWarning[] {
  const warnings: RuleLintWarning[] = [];
  for (const peer of existingLive) {
    if (input.ruleKey !== undefined && peer.ruleKey === input.ruleKey) continue; // never lint a rule against itself
