// Webhook ingestion orchestrator (FR-001/002/003; AD-001/002, HINT-001/002). The verify → dedupe → apply
// pipeline for one delivery, run as ONE tenant transaction so the idempotency claim and the license side
// effect commit atomically (true exactly-once). ORDER (fail-closed, no state change until all pass):
//   1. resolve {connectionId} → tenant (privileged bootstrap, like api-key resolution) — unknown → 404.
//   2. within withTenant(tenant): resolve the connection's UNWRAPPED secret(s) + policy.
//   3. verify the provider HMAC over the RAW body + timestamp recency (current + rotation-window previous
//      secret, constant-time) BEFORE any parse — a failure throws inline (401/400), NO ledger row.
//   4. normalize via the per-provider adapter (malformed → 400); a disabled connection dead-letters.
//   5. map the action, resolve/lock the subscription, apply the stale-event guard, CLAIM the ledger row
//      (INSERT … ON CONFLICT DO NOTHING) and apply the mapped E008 lifecycle change — a duplicate claim is a
//      no-op that re-applies nothing. Genuine faults are BillingErrors; refusals/no-ops are 200 + an outcome.
import { randomUUID } from "node:crypto";

import { privileged, withTenant, type TxQuery } from "../../db/client.js";
import { getAdapter } from "./adapters/index.js";
import { resolveToleranceSecs } from "./config.js";
import { ConnectionRepo, type ResolvedConnection } from "./connection-repo.js";
import { isStaleEvent, mapEventToAction, type CanonicalEvent } from "./events.js";
import type { BillingDeps } from "./index.js";
import { BillingError } from "./index.js";
import { deadLetter, recordEvent, type EventOutcome } from "./ledger-repo.js";
import { applyGrace, applyProvision, applyRenew, applyRevoke, resolveSubscriptionByExternalId } from "./lifecycle.js";
import { getSubscriptionById, type SubscriptionRecord } from "./subscription-repo.js";
import { verifySignature } from "./signature.js";

/** The webhook acknowledgement outcome (contract `AckOutcome`) — refusals/no-ops are 200 + one of these. */
export type AckOutcome = "applied" | "duplicate" | "deadletter";

export interface WebhookInput {
  connectionId: string;
  /** The RAW, unparsed request bytes the provider HMAC was computed over (HINT-001). */
  rawBody: Buffer;
  /**
   * The inbound request headers. The provider signature header NAME is provider-specific (`stripe-signature`
   * / `paddle-signature` / `webhook-signature`), so the orchestrator selects it by the resolved connection's
   * adapter AFTER resolving the connection — a header of the wrong name is simply absent → 401.
   */
  headers: Record<string, string | string[] | undefined>;
  /** Verification/grace clock (epoch seconds); defaults to now. Injected by tests for determinism. */
  nowUnix?: number;
}
...
function evtRow(event: CanonicalEvent, subscriptionId: string | null, outcome: EventOutcome, reason: string | null) {
  return {
...
async function dead(q: TxQuery, event: CanonicalEvent, subscriptionId: string | null, reason: string): Promise<AckOutcome> {
...
async function lockSubscription(q: TxQuery, id: string): Promise<SubscriptionRecord | null> {
...
function signatureFault(reason: string | undefined, toleranceSecs: number): BillingError {
...
async function dispatch(
  ...
  if (event.externalSubscriptionId) {
    ...
    if (resolved) sub = await lockSubscription(q, resolved.id);
  }
  ...
  if (action === "provision") {
    ...
    if (result.duplicate) throw new DuplicateRollback();
    if (!result.applied) return dead(q, event, null, result.reason ?? "unmapped_event");
    const claim = await recordEvent(q, evtRow(event, result.subscriptionId, "applied", null));
    if (claim.duplicate) throw new DuplicateRollback(); // the event id was already processed → roll back the create
    return "applied";
  }

  // --- NON-PROVISION (renew / past_due / cancel / revoke / ignore) ---
  if (!sub) return dead(q, event, null, "unmapped_event"); // no linked subscription
  ...
  if (action === "ignore") return dead(q, event, sub.id, "unmapped_event");

  // renew / past_due / cancel / revoke: claim BEFORE the side effect (exactly-once under concurrent redelivery).
  const claim = await recordEvent(q, evtRow(event, sub.id, "applied", null));
  if (claim.duplicate) return "duplicate";
  if (action === "renew") await applyRenew(deps, q, tenantId, sub, event);
  else if (action === "revoke") await applyRevoke(deps, q, tenantId, sub, event); // FR-010 refund/chargeback → E008 revoke (terminal)
  else await applyGrace(deps, q, tenantId, conn, sub, event, action, now);
  return "applied";
}
 ...
 */
export async function handleWebhook(deps: BillingDeps, input: WebhookInput): Promise<WebhookResult> {
  ...
  const owner = await privileged(deps.pool, (q) =>
    q("SELECT tenant_id FROM billing_connection WHERE id = $1", [input.connectionId]),
  );
  if (!owner.rowCount) {
  ...
  try {
    const outcome = await withTenant(deps.pool, tenantId, async (q): Promise<AckOutcome> => {
      ...
      if (!conn) throw new BillingError("connection_not_found", 404, "unknown connection", { connectionId: input.connectionId });
      ...
      if (!verified.ok) throw signatureFault(verified.reason, toleranceSecs);
      ...
      if (!event) throw new BillingError("validation_error", 400, "the webhook body could not be parsed");

      // A disabled connection still verifies (verify-before-process) but applies nothing → dead-letter.
      if (conn.status === "disabled") return dead(q, event, null, "connection_disabled");
    ...
    if (e instanceof DuplicateRollback) return { outcome: "duplicate" };
    throw e;
  }
}
