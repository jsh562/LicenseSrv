...
export type ProvisionFirstAdminOutcome =
  | { ok: true; id: string }
  | { ok: false; reason: "duplicate" };
...
): Promise<ProvisionFirstAdminOutcome> {
  const id = randomUUID();
  try {
    await withTenant(pool, tenantId, async (q) => {
      await q(
        `INSERT INTO app_user (id, tenant_id, email_hash, status)
         VALUES ($1, current_setting('app.current_tenant')::uuid, $2, 'active')`,
        [id, userReference],
      );
      await q(
        `INSERT INTO role (id, tenant_id, user_id, role, granted_by)
         VALUES ($1, current_setting('app.current_tenant')::uuid, $2, 'owner', $3)`,
        [randomUUID(), id, actor],
      );
      await writeAudit(q, {
        actor,
