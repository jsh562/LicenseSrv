...
import { validateOnline, type EnforcementResult, type ValidateInput } from "./validate.js";
...
async function guard(
  ...
  onDenied?: (e: EnforcementError) => Promise<void>,
...
function requireValidateScope(req: FastifyRequest, reply: FastifyReply): boolean {
  if (!req.tenant) {
...
function toWire(r: EnforcementResult): Record<string, unknown> {
  const body: Record<string, unknown> = {
    verdict: r.verdict,
    serverTime: r.serverTime,
    stalenessWindow: r.stalenessWindow,
  };
  if (r.shortLivedToken !== undefined) body.shortLivedToken = r.shortLivedToken;
  if (r.renewAfter !== undefined) body.renewAfter = r.renewAfter;
  if (r.expiresAt !== undefined) body.expiresAt = r.expiresAt;
  return body;
}
...
async function auditRateLimited(pool: pg.Pool, secret: string, req: FastifyRequest): Promise<void> {
  ...
  await withTenant(pool, ctx.tenantId, (q) =>
    recordSecurityEvent(q, { actor: ACTOR, action: "enforcement.rate_limited", target: `${req.method} ${req.url}` }),
  );
}

export interface EnforcementRouteDeps {
  config: EnforcementConfig;
  signer: Signer | undefined;
  apiKeySecret: string;
}

/** The shared runtime handler for validate + heartbeat (SAME auth/scope/schema/response; FR-003/004). */
type EnforcementRunner = (
  pool: pg.Pool,
  signer: Signer | undefined,
  config: EnforcementConfig,
  tenantId: string,
...
export function registerEnforcementRoutes(app: FastifyInstance, pool: pg.Pool, deps: EnforcementRouteDeps): void {
  ...
  const handler =
    (run: EnforcementRunner) =>
    async (req: FastifyRequest, reply: FastifyReply): Promise<FastifyReply> => {
      if (!requireValidateScope(req, reply)) return reply;
      const tenantId = req.tenant!.tenantId;
      const b = validateSchema.safeParse(req.body);
      if (!b.success) return validation(reply, b.error.issues[0]?.message ?? "invalid enforcement payload");
      return guard(
        reply,
        async () => {
  ...
  void app.register(async (scope) => {
    ...
    scope.get("/v1/revocation-list", rl, async (req, reply): Promise<FastifyReply> => {
      if (!requireValidateScope(req, reply)) return reply;
      const tenantId = req.tenant!.tenantId;
      const parsed = crlQuerySchema.safeParse(req.query);
      if (!parsed.success) return validation(reply, parsed.error.issues[0]?.message ?? "invalid revocation-list query");
