...
export function effectiveExpiry(
  ...
  credentialTtlSecs: number | null,
  nowUnix: number,
): number | null {
  const candidates: number[] = [];
  if (licenseExpiresAt) candidates.push(Math.floor(new Date(licenseExpiresAt).getTime() / 1000));
  if (credentialTtlSecs != null) candidates.push(nowUnix + credentialTtlSecs);
  return candidates.length ? Math.min(...candidates) : null;
}

export interface MachineClaimsInput {
  license: License;
  signalHashes: string[];
  fpMin: number;
  maxSkewSecs: number;
...
export function buildMachineClaims(input: MachineClaimsInput): Claims {
  const { license } = input;
  return {
