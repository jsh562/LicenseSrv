...
export function deriveMachineId(signalHashes: string[], salt: string): string {
...
export function overlapCount(a: string[], b: string[]): number {
  ...
  let n = 0;
  for (const s of new Set(a)) if (setB.has(s)) n++;
  return n;
}

/** An active activation row considered as a re-match candidate. */
