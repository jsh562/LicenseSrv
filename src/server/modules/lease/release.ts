...
import type { LeaseDeps } from "./index.js";

const ACTOR = "lease-api";

/** The idempotent release result — the echoed lease id + the terminal `released` status (always 200). */
 ...
 */
export async function releaseLease(deps: LeaseDeps, tenantId: string, leaseId: string): Promise<ReleaseResult> {
