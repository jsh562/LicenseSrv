...
async function guard(
  ...
  onDenied?: (e: LeaseError) => Promise<void>,
...
function requireLeaseScope(req: FastifyRequest, reply: FastifyReply): boolean {
  if (!req.tenant) {
...
/** One lease as shown in the admin registry (contract `LeaseSummary`): pseudonymous, read-only, NO handle. */
interface LeaseSummary {
  id: string;
  holderKey: string;
  scope: LeaseRecord["scope"];
  status: LeaseStatus;
  acquiredAt: string;
...
function toSummary(lease: LeaseRecord): LeaseSummary {
  return {
    id: lease.id,
...
async function auditRateLimited(pool: pg.Pool, secret: string, req: FastifyRequest): Promise<void> {
  ...
  await withTenant(pool, ctx.tenantId, (q) =>
    recordSecurityEvent(q, { actor: "lease-api", action: "lease.rate_limited", target: `${req.method} ${req.url}` }),
  );
}

/** Register the /v1 runtime lease routes (acquire / renew / release) — API key + `lease` scope + rate limit. */
export function registerLeaseRoutes(app: FastifyInstance, deps: LeaseDeps, apiKeySecret: string): void {
  ...
  void app.register(async (scope) => {
    ...
    scope.post("/v1/leases", rl, async (req, reply) => {
      if (!requireLeaseScope(req, reply)) return reply;
      const tenantId = req.tenant!.tenantId;
      const b = acquireSchema.safeParse(req.body);
      if (!b.success) return validation(reply, b.error.issues[0]?.message ?? "invalid acquire payload");
      return guard(
        reply,
        async () => {
          ...
          if (created) r.header("Location", `/v1/leases/${grant.id}`);
          return r.send(grant);
        },
    ...
    scope.post("/v1/leases/:leaseId/renew", rl, async (req, reply) => {
      ...
      if (!p.success) return validation(reply, "invalid leaseId");
      return guard(
        reply,
        async () => {
 ...
 * plane above.
 */
function registerLeaseAdminRoutes(app: FastifyInstance, deps: LeaseDeps): void {
  ...
  void app.register(async (adminScope) => {
    ...
    adminScope.get("/admin/licenses/:licenseId/leases", viewer, async (req, reply) => {
      ...
      if (!qy.success) return validation(reply, "invalid status filter");
      ...
      if (!result) return err(reply, 404, "not_found", "unknown license", { licenseId: p.data.licenseId });
    ...
    adminScope.post("/admin/leases/:leaseId/force-release", admin, async (req, reply) => {
      ...
      const result = await withTenant(pool, tenantId, async (q) => {
        ...
        if (!existing) return null; // unknown / cross-tenant → 404 (FR-019)
      ...
      if (!result) return err(reply, 404, "not_found", "unknown lease", { leaseId: p.data.leaseId });
      return reply.code(200).send(result);
    });
  });
}
