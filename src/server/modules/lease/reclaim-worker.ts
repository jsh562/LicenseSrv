// Lease reclaim worker (E015, FR-010/018/024; ADR-0012 AD-002/AD-007, HINT-004). A time-driven, fail-open,
// unref'd sweeper — modeled on the E013 CRL / E014 grace-reclaim workers — that returns dead-machine seats to
// the pool with no operator action. Each run drains, per tenant, TWO bounded oldest-expired-first batches:
//   1. TTL + grace-lapsed live leases (`expires_at + grace < now`) — the dead-machine reclaim (FR-010).
//   2. LIVE leases whose license state triggers the CONFIGURABLE per-reason `reclaim` policy (FR-024) —
//      proactive reclaim regardless of TTL so the seats free within the sweep interval, near-immediate:
//        * license `revoked`   AND `lease_policy_on_revoke  = 'reclaim'` (DEFAULT reclaim)  → `license_revoked`
//        * license `suspended` AND `lease_policy_on_suspend = 'reclaim'` (default timer)     → `license_suspended`
//        * license past `expires_at` AND `lease_policy_on_expire = 'reclaim'` (default timer)→ `license_expired`
//      The DEFAULTS are unchanged (revoke⇒reclaim, suspend/expire⇒timer): a suspend/expire only proactively
//      reclaims when its column is EXPLICITLY set to `reclaim`, otherwise its leases lapse on timer via #1.
// Every reclamation is audited with a SYNTHETIC worker actor + the lease/license id + the matching reason
// (FR-018). Fail-open: a fault on one tenant never aborts the others and NO fault ever throws out of the sweep
// or crashes the app — reclamation must never block the live acquire/renew/release surface (SC-008).
// [COMPLETES FR-024]
...
import { LeaseRepo, type ReclaimedLease } from "./lease-repo.js";
...
export interface ReclaimWorkerLogger {
  warn(obj: object, msg?: string): void;
}

/** Options for a single {@link reclaimSweep}. */
export interface ReclaimSweepOptions {
  /** Bounded leases reclaimed per (tenant, path) per run, oldest-expired-first (FR-010). */
  maxBatch?: number;
  /** Optional structured logger for fail-open warnings. */
  logger?: ReclaimWorkerLogger;
  /** Optional hook invoked with the error on a per-tenant / sweep failure (diagnostics/tests). */
  onError?: (err: unknown) => void;
}

/** The outcome of one full sweep across all tenants. */
...
/** The distinct per-reason live-lease reclaim reasons the configurable FR-024 policy attributes (FR-018). */
export type PolicyReclaimReason = "license_revoked" | "license_suspended" | "license_expired";

/** A policy-reclaimed lease + the license-state reason it was reclaimed for (drives the synthetic-actor audit). */
interface PolicyReclaimedLease extends ReclaimedLease {
  reason: PolicyReclaimReason;
}
 ...
 * actual rows.
 */
async function listTenantsWithDueLeases(pool: pg.Pool): Promise<string[]> {
 ...
 */
async function reclaimByPolicy(q: TxQuery, maxBatch: number): Promise<PolicyReclaimedLease[]> {
  const res = await q(
    `WITH due AS (
       SELECT l.id,
              CASE
     ...
     SELECT r.id, r.license_id, d.reason
       FROM reclaimed r JOIN due d ON d.id = r.id`,
    [maxBatch],
  );
  return (res.rows as { id: string; license_id: string; reason: PolicyReclaimReason }[]).map((r) => ({
    id: r.id,
 ...
 */
export async function reclaimSweep(pool: pg.Pool, options: ReclaimSweepOptions = {}): Promise<ReclaimSweepResult> {
  const maxBatch = options.maxBatch ?? DEFAULT_SWEEP_MAX_BATCH;
  const warn = (err: unknown, context: string): void => {
    try {
      options.logger?.warn(
        { event: "reclaim_worker_failed", context, error: err instanceof Error ? err.message : String(err) },
        "lease reclaim step failed (fail-open); the live lease surface is unaffected",
      );
    } catch {
...
export interface ReclaimWorkerOptions extends ReclaimSweepOptions {
  /** Cadence in ms; default {@link DEFAULT_RECLAIM_WORKER_INTERVAL_MS}. */
  intervalMs?: number;
  /** Run one sweep immediately on start; default true. Tests pass false and drive `runOnce` deterministically. */
  immediate?: boolean;
}

/** A started reclaim worker. `stop()` cancels the cadence; `runOnce()` runs a single fail-open sweep. */
export interface ReclaimWorkerHandle {
  stop(): void;
 ...
 */
export function startReclaimWorker(pool: pg.Pool, options: ReclaimWorkerOptions = {}): ReclaimWorkerHandle {
