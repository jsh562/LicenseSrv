...
const core = require("../../../bindings/wasm/pkg/licensesrv.js") as {
  Keyring: new () => { add(keyId: string, publicKey: Uint8Array): number; free(): void };
  verify: (
...
/** The signed license payload (mirrors the core's `Claims`; CBOR field names in comments). */
export interface Claims {
  tokenVersion: number; // v
  licenseId: string; // lid
  productId: string; // pid
  planId: string; // pl
  customerId: string; // cid
  issuedAt: number; // iat
  expiresAt?: number | null; // exp (omitted when null)
  maxActivations: number; // maxa
  fingerprint?: string[] | null; // fp
  fpMin?: number | null; // fpk
  maxSkewSecs?: number | null; // sk
  entitlements: Record<string, boolean | number>; // ent
  maxVersion?: string | null; // maxv
  maintenanceUntil?: number | null; // mnt
  keyId: string; // kid
  nonce: string; // non
}
...
const cborInt = (n: number): Buffer => (n >= 0 ? cborHead(0, n) : cborHead(1, -n - 1));
function cborText(s: string): Buffer {
  const b = Buffer.from(s, "utf8");
  return Buffer.concat([cborHead(3, b.length), b]);
}
const cborBool = (v: boolean): Buffer => Buffer.from([v ? 0xf5 : 0xf4]);
// entitlements is a BTreeMap in the core -> CBOR map with keys in sorted order.
function cborEntitlements(ent: Record<string, boolean | number>): Buffer {
...
export function encodeClaims(c: Claims): Buffer {
  ...
  if (c.expiresAt != null) fields.push(["exp", cborInt(c.expiresAt)]);
  ...
  if (c.fpMin != null) fields.push(["fpk", cborInt(c.fpMin)]);
  if (c.maxSkewSecs != null) fields.push(["sk", cborInt(c.maxSkewSecs)]);
  fields.push(["ent", cborEntitlements(c.entitlements)]);
  if (c.maxVersion != null) fields.push(["maxv", cborText(c.maxVersion)]);
  if (c.maintenanceUntil != null) fields.push(["mnt", cborInt(c.maintenanceUntil)]);
...
export function buildSigningInput(claims: Claims): { payload: Buffer; signingInput: Buffer } {
...
export function assembleToken(payload: Buffer, signature: Buffer): string {
